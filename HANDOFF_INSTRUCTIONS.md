...
- **JSX を正規表現で走査するとき `>` で止まらない**: 属性に `onChange={(e) => ...}` が入るため、
  `<input[^>]*>` は矢印関数の `>` で切れ、**後ろにある `maxLength` を見落として「上限なし」と誤報**する
  （#102 で 47 件の誤ったリストを生成した）。波括弧の深さと引用符を追って閉じ `>` を探すこと。
  `tests/test_stated_policy_matches_enforcement.py` の `jsx_elements()` が実装済み。
- **フロントの型依存は宣言する**: `src/` 配下のテストが `node:fs`/`process` を使うなら
