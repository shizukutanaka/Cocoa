...
### 3-6. 同種の「構成状態を500で報告」候補（#53 で発見、未修正）

#53 と同じパターンが残る箇所。いずれも `except Exception` → `logger.error` → 500 で、原因が env/構成の可能性が高い。**再現条件を確証できていないため推測で修正せず記録に留める**（独立ラウンドの候補）:

| エンドポイント | 位置 | 懸念 |
|---|---|---|
| `GET /backups` | `api_server.py`（`get_recovery_manager()` 構築/保存先設定） | 未構成で 500。**管理者ゲートがなく一般ログインユーザーが到達可能** |
| `GET /security/report` | 同上（`get_security_manager().initialize()`） | 同上。未構成のセキュリティスタックで 500 の可能性 |
