...
| 項目 | 内容 | 規模 |
|---|---|---|
| ~~**管理・モデレーション UI 皆無**~~ **→ #46 / #49 / #62 で解消進行中** | 通報裁定・レビュー通報・払い戻し承認は `#46`、クリエイター認定申請の審査は `#49` で配線済み（重大度トリアージ・判断理由の必須化・復元導線・実行者記録つき）。BAN／出品者履歴は `#50`、通報者への結果通知は `#61`、**ユーザー名簿・クレジット付与は `#62`** で配線済み。**残りの未接続 admin エンドポイント**（モデレーションキュー・会員ティア調整・監査エクスポート・ロール変更・クオータ設定等）は引き続き未到達 | 中（続きは別ラウンド） |
