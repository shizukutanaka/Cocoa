...
| 項目 | 内容 | 規模 |
|---|---|---|
| ~~**管理・モデレーション UI 皆無**~~ **→ #46 / #49 で解消** | 通報裁定・レビュー通報・払い戻し承認は `#46`、クリエイター認定申請の審査は `#49` で配線済み（重大度トリアージ・判断理由の必須化・復元導線・実行者記録つき）。**残りの未接続 admin エンドポイント**（ユーザー管理・BAN・クレジット付与・モデレーションキュー・会員ティア調整・監査エクスポート等）は引き続き未到達 | 中（続きは別ラウンド） |
