...
- `/api/marketplace/*`（拡張）: reviews（投稿/一覧/削除）, report, analytics/me, trending-tags
- `/api/collections/*`: CRUD + アイテム追加/削除 + 公開コレクション閲覧
- `/api/admin/reports/*`: モデレーション通報一覧/統計/解決（テイクダウン）
- `/live`, `/ready`: liveness / readiness プローブ

### セキュリティ強化

- 全レスポンスにセキュリティヘッダ付与（X-Content-Type-Options, X-Frame-Options,
  Referrer-Policy, X-XSS-Protection, Permissions-Policy）
- レート制限キーを認証ユーザーID優先に変更（NAT 配下のユーザーを個別バケット化、
  匿名は IP フォールバック）
- WebSocket `/ws/monitoring` に JWT 認証
...
python tests/run_safe.py       →  1597 passed, 34 skipped
API エンドポイント総数            →  74 本
```
